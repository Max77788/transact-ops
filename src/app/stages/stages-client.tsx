    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="rounded-xl p-6 w-full max-w-md" style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold" style={{ color: "var(--text)", fontFamily: "Instrument Serif, serif" }}>Add Stage</h3>
          <button onClick={onClose} style={{ color: "var(--text3)" }}><X size={18} /></button>
        </div>
        <div className="space-y-3">
          <input className="w-full rounded-lg px-3 py-2.5 text-sm border" style={{ backgroundColor: "var(--bg)", color: "var(--text)", borderColor: "var(--border)" }} placeholder="Stage name *" value={name} onChange={(e) => setName(e.target.value)} />
          <input className="w-full rounded-lg px-3 py-2.5 text-sm border" style={{ backgroundColor: "var(--bg)", color: "var(--text)", borderColor: "var(--border)" }} placeholder="Index (0-10) *" type="number" value={idx} onChange={(e) => setIdx(e.target.value)} />
          <input className="w-full rounded-lg px-3 py-2.5 text-sm border" style={{ backgroundColor: "var(--bg)", color: "var(--text)", borderColor: "var(--border)" }} placeholder="Description (optional)" value={desc} onChange={(e) => setDesc(e.target.value)} />
        </div>
...
function AddStepModal({ stageId, onClose, onCreated }: { stageId: string; onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.6)" }} onClick={onClose}>
      <div className="rounded-xl p-6 w-full max-w-md" style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold" style={{ color: "var(--text)", fontFamily: "Instrument Serif, serif" }}>Add Step</h3>
          <button onClick={onClose} style={{ color: "var(--text3)" }}><X size={18} /></button>
        </div>
        <div className="space-y-3">
          <input className="w-full rounded-lg px-3 py-2.5 text-sm border" style={{ backgroundColor: "var(--bg)", color: "var(--text)", borderColor: "var(--border)" }} placeholder="Step name *" value={name} onChange={(e) => setName(e.target.value)} />
          <input className="w-full rounded-lg px-3 py-2.5 text-sm border" style={{ backgroundColor: "var(--bg)", color: "var(--text)", borderColor: "var(--border)" }} placeholder="Order (0-N) *" type="number" value={idx} onChange={(e) => setIdx(e.target.value)} />
        </div>
        {error && <p className="text-xs mt-3" style={{ color: "var(--high)" }}>{error}</p>}
        <button className="w-full mt-4 rounded-lg py-2.5 text-sm font-semibold" style={{ backgroundColor: "var(--accent)", color: "var(--bg)" }} onClick={submit} disabled={submitting}>
...
export function StagesClient({ initialStages }: { initialStages: DBStage[] }) {
  ...
  return (
    <div className="px-4 sm:px-6 pt-4 pb-6 space-y-4">
      ...
      {sorted.length === 0 ? (
        <p className="text-sm" style={{ color: "var(--text3)" }}>No stages configured.</p>
      ) : (
        sorted.map((stage) => (
          <div key={stage.id} className="rounded-lg p-4" style={{ backgroundColor: "var(--surface)", border: "1px solid var(--border)" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium" style={{ color: "var(--text)" }}>{stage.name}</span>
              {stage.description && (
                <span className="text-[11px]" style={{ color: "var(--text3)" }}>— {stage.description}</span>
              )}
              <button
            ...
            {stage.steps && stage.steps.length > 0 ? (
              <div className="space-y-1.5">
                {[...stage.steps].sort((a, b) => a.idx - b.idx).map((step) => (
                  <div key={step.id} className="flex items-center gap-2 px-3 py-2 rounded-md text-sm" style={{ color: "var(--text2)" }}>
